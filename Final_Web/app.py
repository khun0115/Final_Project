...
def out_put():
    ...
    return render_template('out_put.html', part_output=part_output, damage_type_output=damage_type_output)#, hq_output 
...
def damage_type_model_predict():
    ...
    model = tf.keras.models.load_model('Final_Web/static/models/car_damage_type.h5')
    # 로딩된 모델 사용 예시
...
def HQ_ML_model(part_output, damage_type_output):
    model = tf.keras.models.load_model('Final_Web/static/models/hq_time.h5')
    # 로딩된 모델 사용 예시
    X_pred = [part_output, damage_type_output]
    hq_output = model.predict(X_pred)
    return hq_output
